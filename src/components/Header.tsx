  ...
  return (
    <>
      ...
      <div className="bg-primary text-white text-xs py-1.5 px-4 text-center font-medium flex items-center justify-center gap-2">
        ...
        <span>الدفع عند الاستلام متاح لجميع الـ 58 ولاية جزائرية • توصيل سريع وموثوق</span>
        <span className="hidden sm:inline bg-secondary/30 px-2 py-0.5 rounded-full text-[11px] font-bold">100% أصلي ومضمون</span>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between gap-4">
          ...
          <Link href="/" className="flex items-center gap-3 shrink-0 group">
            ...
            <div className="flex flex-col">
              <span className="font-serif text-xl sm:text-2xl font-black tracking-tight text-primary">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <form onSubmit={handleSearch} className="hidden md:flex items-center relative">
              <input
                ...
                className="w-44 lg:w-56 bg-surface-container-low text-on-surface text-sm pr-9 pl-4 py-2 rounded-full outline-none focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all"
              />
              <button type="submit" aria-label="بحث" className="absolute right-3 text-outline hover:text-primary transition-colors">
            ...
            <Link
              ...
              className="relative w-11 h-11 rounded-full bg-surface-container-low hover:bg-surface-container flex items-center justify-center text-primary transition-all duration-200 shadow-sm"
            >
              <ShoppingBag className="w-5 h-5" />
              {totalItems > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-secondary text-white text-xs font-black flex items-center justify-center ring-2 ring-white animate-pulse">
                  {totalItems}
                </span>
              )}
            </Link>
